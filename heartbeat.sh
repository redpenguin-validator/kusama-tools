...
YOUR_STASH_ADDRESS="G7mWyu1Pom5XreLHUzDEcvFp6WaMuLuo4QKxtDB9yJZnH69"
...
index=$(curl -s "http://$YOUR_API_NODE/api/query/session/validators?websocket=ws://$YOUR_WS" | jq -r '.[] | to_entries[] | select(.value=="'$YOUR_STASH_ADDRESS'") | [.key] | @tsv ')
